...
> **\*效率说明**: go-tpc 的效率公式为 `tpmC / (12.86 × warehouses) × 100%`，其中 12.86 tpmC/warehouse 是官方 TPC-C 规范中单 warehouse 的最大理论吞吐。基准环境下 12,150% 的效率看起来异常，原因有二：(1) go-tpc 未完整实现 TPC-C 的严格 ACID 约束（如 keying time、think time），允许更高的吞吐；(2) 分布式数据库的多节点并行执行超出了单节点吞吐模型的设计范围。因此**效率值在此无实际意义**，仅供参考。
